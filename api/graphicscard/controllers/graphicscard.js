...
module.exports = {
  async find(ctx) {
    ...
    if (ctx.query._q) {
      entities = await strapi.services.graphicscard.search(ctx.query);
    } else {
      /* console.log(ctx.query); */
      entities = await strapi.services.graphicscard.find(ctx.query);
    }

    let res = entities.map((entity) => {
      return sanitizeEntity(entity, { model: strapi.models.graphicscard });
    });
  ...
  async filter() {
    ...
    let entities = await strapi.services.graphicscard.find();

    entities.map((item) => {
      filter_values[0].values.push(item.creator);
